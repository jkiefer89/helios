...
from . import (
    cma, data, forecast, fundamentals, indicators, opportunity, portfolio,
    portfolio_clinic, provenance, research_context, risk_exposure, sentiment,
    signals, strategy,
)
...
def instrument_report(inst) -> dict:
    ...
    try:
        ...
        oos = strategy.analyze_oos_evidence(close)
        strategy_note = ""
    ...
    return _base_report(
        ...
        sections={
            ...
            "strategy": _strategy_summary(st, oos) if st else _strategy_unavailable(strategy_note),
...
def model_report(
    ...
    *,
    ...
    context = research_context.model_context(model)
    freshness = data.model_freshness(model)
    ...
    oos = strategy.analyze_oos_evidence(ps.close)
    ...
    return _base_report(
        ...
        sections={
            ...
            "strategy": _strategy_summary(st, oos),
            "research_context": context,
            "clinic": {
                "diagnostics": clinic["diagnostics"],
                "suggestions": clinic["suggestions"],
...
def _strategy_summary(st: dict, oos_evidence: dict | None = None) -> dict:
    return {
        "available": True,
        ...
        "current_signal": st.get("current_signal", {}),
        "path_evidence": st.get("path_evidence", {}),
        "oos_evidence": dict(oos_evidence or {}),
