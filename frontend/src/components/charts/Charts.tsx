...
export function ScoreBar({ value, tone = "positive" }: { value?: number; tone?: string }) {
  ...
  return (
    <div className="score-bar" aria-label={`Score ${fmtNumber(safe, 1)} out of 100`}>
      <span className={`score-bar__fill tone-${tone}`} style={{ width: `${safe}%` }} />
      ...
      {rows.map((row) => {
        ...
        return (
          <div className="mini-bars__row" key={row.label}>
            <span>{row.label}</span>
            <div><i className={`tone-${row.tone || "positive"}`} style={{ width: `${safe}%` }} /></div>
...
export function LineChart({
  ...
  return (
    <div className="line-chart" style={{ height }}>
      <svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Historical line chart">
        <g className="grid-lines">
          {[0.25, 0.5, 0.75].map((pct) => <line key={pct} x1="0" x2={width} y1={height * pct} y2={height * pct} />)}
        </g>
        {series.map((item) => {
          const values = item.values
            .map((value, index) => (typeof value === "number" && Number.isFinite(value) ? toPoint(value, index, item.values.length) : null))
            .filter(Boolean)
            .join(" ");
          return <polyline key={item.label} className={`line-chart__line tone-${item.tone || "info"}`} points={values} />;
        })}
      </svg>
      <div className="chart-legend">
        {series.map((item) => <span key={item.label} className={`tone-${item.tone || "info"}`}>{item.label}</span>)}
      </div>
    </div>
  );
}
